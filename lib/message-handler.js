...
MessageHandler.prototype._asyncCallback = function (callback_id, error, result, result2) {
  ...
  self._async_responses.push(new AsyncResponse(callback_id, responseString));
};
...
MessageHandler.prototype._syncCallback = function (error, result, result2) {
  ...
  var responseString = JSON.stringify(response);
  var headerBuffer = createResponseHeader();

  self._stdio[3].write(headerBuffer);
  self._stdio[3].write(responseString);
};

function createResponseHeader(asyncResponse) {
