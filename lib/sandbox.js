...
function Sandbox(options) {
  ...
  self.options = {};
  options = options || {};
  self.options.timeout = options.timeout || 500;
  self.options.node = options.node || 'node';
  self.options.shovel = options.shovel || path.join(__dirname, 'shovel.js');
  self.options.sandbox_filesystem_path = options.sandbox_filesystem_path;
  ...
  self._stdout = options.output_stream || process.stdout;
}
...
Sandbox.prototype.run = function(manifest_hash, code, callback) {
  var self = this;
  var timer;
  var result;
  var error = null;
  ...
  // Spawn child process
  self.child = spawn(this.options.node, [
    self.options.shovel,
    self.options.sandbox_filesystem_path,
    manifest_hash
  ...
  self.child.stderr.on('data', stderrHandler);
