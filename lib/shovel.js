...
var fsreadonly = new FileSystemReadOnly(sandbox_filesystem_path);
var manifest;
try {
  ...
  try {
    safeRunner({
      ...
      __readFileSync: function(path) {
        ...
        return fsreadonly.readFileSync(manifest, String(path));
      },
      Int32Array: Buffer
    }, code);
  }
  catch (e) {
    process.stdout.write(String(e.name) + ': ' + String(e.message));
    return;
  }
